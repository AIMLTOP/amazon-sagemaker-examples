...
def train(args):
    ...
    wandb.watch(model) # add for wandb
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_idx, (data, target) in enumerate(train_loader, 1):
            ...
            optimizer.step()
            wandb.log({"training/loss": loss.item()}) # add for wandb
...
def test(model, test_loader, device):
    ...
    wandb.log({"testing/loss": test_loss}) # add for wandb
    logger.info(
...
if __name__ == "__main__":
    ...
    wandb.init(project="sm-pytorch-mnist-wandb", config=vars(args))
    train(args)
